...
func Normalize(address string, nativeAsset xc_types.NativeAsset) string {
	...
	switch blockchain := xc_types.NativeAsset(nativeAsset).Protocol(); blockchain {
	case xc_types.ProtocolEVM, xc_types.ProtocolEVMLegacy:
		prefix := "0x"
	...
	case xc_types.ProtocolBtcCash, xc_types.ProtocolBtc:
		// remove bitcoincash: prefix
		if strings.Contains(address, ":") {
			address = strings.Split(address, ":")[1]
		}
	/*
		case xc_types.ProtocolAptos, xc_types.ProtocolSui:
			address = NormalizeMoveAddress(address)
			if options.NoPrefix {
				address = strings.TrimPrefix(address, "0x")
			}
	*/
	case xc_types.ProtocolCosmos:
		// nothing to do, bech32

	case xc_types.ProtocolSolana:
		// nothing to do, base58
	case xc_types.ProtocolTron:
		// TODO
	case xc_types.ProtocolTon:
