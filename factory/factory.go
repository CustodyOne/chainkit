...
import (
	...
	remoteclient "github.com/CustodyOne/chainkit/blockchain/chainkit"
	"github.com/CustodyOne/chainkit/builder"
...
func (f *Factory) NewClient(cfg *types.ChainConfig) (xc_client.IClient, error) {
	...
	switch xc.Protocol(client.Protocol) {
	case xc.ProtocolChainkit:
		return remoteclient.NewClient(cfg, client.Auth)
	default:
		return protocols.NewClient(cfg, xc.Protocol(client.Protocol))
	}
}
...
func (f *Factory) GetAllPossibleAddressesFromPublicKey(cfg *types.ChainConfig, publicKey []byte) ([]types.PossibleAddress, error) {
	builder, err := protocols.NewAddressBuilder(cfg)
	if err != nil {
