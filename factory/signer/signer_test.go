...
func TestNewSigner(t *testing.T) {
	...
	s, err := signer.New(xc.ProtocolEVM, privateKey, nil)
	require.NoError(t, err)
	require.NotNil(t, s)

	privateKey = "12345678"
	_, err = signer.New(xc.ProtocolEVM, privateKey, nil)
	require.Error(t, err)
	...
	_, err = signer.New(xc.ProtocolCosmos, mnemonic, nil)
	require.NoError(t, err)
}
