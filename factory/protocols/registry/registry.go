...
func RegisterTxBaseInput(txInput xc.TxInput) {
	for _, existing := range supportedBaseInputTx {
		if existing.GetProtocol() == txInput.GetProtocol() {
			panic(fmt.Sprintf("base input %T protocol %s duplicates %T", txInput, txInput.GetProtocol(), existing))
		}
	}
...
func RegisterTxVariantInput(variant xc.TxVariantInput) {
	for _, existing := range supportedVariantTx {
		if existing.GetVariant() == variant.GetVariant() {
			panic(fmt.Sprintf("staking input %T protocol %s duplicates %T", variant, variant.GetVariant(), existing))
		}
	}
