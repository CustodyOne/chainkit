...
func MarshalTxInput(methodInput xc.TxInput) ([]byte, error) {
	...
	if variant, ok := methodInput.(xc.TxVariantInput); ok {
		...
		data[SerializedInputTypeKey] = methodInput.GetProtocol()
	}
...
func NewTxInput(protocol xc.Protocol) (xc.TxInput, error) {
	for _, txInput := range registry.GetSupportedBaseTxInputs() {
		if txInput.GetProtocol() == protocol {
			return makeCopy(txInput), nil
		}
		// aliases for fork chains
		switch protocol {
		case xc.ProtocolBtc, xc.ProtocolBtcCash, xc.ProtocolBtcLegacy:
			if txInput.GetProtocol() == xc.ProtocolBtc {
				return makeCopy(txInput), nil
			}
		case xc.ProtocolCosmos, xc.ProtocolCosmosEvmos:
			if txInput.GetProtocol() == xc.ProtocolCosmos {
				return makeCopy(txInput), nil
			}
		}
	}

	return nil, fmt.Errorf("no tx-input mapped for protocol %s", protocol)
}
