...
import (
	...
	"github.com/CustodyOne/chainkit/blockchain/cosmos/address"
	"github.com/CustodyOne/chainkit/blockchain/cosmos/types/evmos/ethermint/crypto/ethsecp256k1"
...
func TestGetAddressFromPublicKeyEvmos(t *testing.T) {
	builder, _ := address.NewAddressBuilder(&xc.ChainConfig{Chain: "XPLA", ChainPrefix: "xpla", Protocol: xc.ProtocolCosmosEvmos})
...
func TestKeyDerivation(t *testing.T) {
	...
	for _, tc := range []testcase{
		...
		},
		{
			...
			Protocol:        xc.ProtocolCosmosEvmos,
		},
		{
			ChainCoinHDPath: 60,
			ChainPrefix:     "xpla",
		...
		s, err := signer.New(tc.NativeAsset.Protocol(), tc.Mnemonic, asset)
		require.NoError(t, err)
		...
		if tc.Address != string(derivedAddress) {
			...
			for i := 0; i < 512; i++ {
				...
				s, _ = signer.New(tc.NativeAsset.Protocol(), tc.Mnemonic, asset)
...
func TestIsEVMOS(t *testing.T) {
	is := address.IsEVMOS(&xc.ChainConfig{Chain: "ETH", Protocol: xc.ProtocolEVM})
	require.False(t, is)

	is = address.IsEVMOS(&xc.ChainConfig{Chain: "ATOM", Protocol: xc.ProtocolCosmos})
	require.False(t, is)

	is = address.IsEVMOS(&xc.ChainConfig{Chain: "LUNA", Protocol: xc.ProtocolCosmos})
	require.False(t, is)

	is = address.IsEVMOS(&xc.ChainConfig{Chain: "XPLA", Protocol: xc.ProtocolCosmos})
	require.False(t, is)

	is = address.IsEVMOS(&xc.ChainConfig{Chain: "XPLA", Protocol: xc.ProtocolCosmosEvmos})
	require.True(t, is)
}

func TestGetPublicKey(t *testing.T) {
	pubKey := address.GetPublicKey(&xc.ChainConfig{Protocol: xc.ProtocolCosmos}, []byte{})
	require.Exactly(t, &secp256k1.PubKey{Key: []byte{}}, pubKey)

	pubKey = address.GetPublicKey(&xc.ChainConfig{Protocol: xc.ProtocolCosmosEvmos}, []byte{})
