...
import (
	...
	"github.com/CustodyOne/chainkit/blockchain/btc/address"
	"github.com/CustodyOne/chainkit/blockchain/btc/tx"
	"github.com/CustodyOne/chainkit/blockchain/btc/tx_input"
...
func (s *ChainkitTestSuite) TestGetAddressFromPublicKeyUsesCompressed() {
	require := s.Require()
	builder, err := address.NewAddressBuilder(&xc.ChainConfig{
		Network:  "testnet",
		Chain:    xc.BTC,
		Protocol: xc.ProtocolBtc,
	})
	require.NoError(err)
...
func (s *ChainkitTestSuite) TestNewNativeTransfer() {
	require := s.Require()
...
func (s *ChainkitTestSuite) TestNewTokenTransfer() {
	require := s.Require()
...
func (s *ChainkitTestSuite) TestNewTransfer() {
	require := s.Require()
	asset := &xc.ChainConfig{Chain: xc.BTC, Network: "testnet"}
	builder, _ := NewTxBuilder(asset)
	from := xc.Address("mpjwFvP88ZwAt3wEHY6irKkGhxcsv22BP6")
...
func (s *ChainkitTestSuite) TestNewTransfer_Token() {
	require := s.Require()
...
func (s *ChainkitTestSuite) TestTxHash() {
	require := s.Require()
...
func (s *ChainkitTestSuite) TestTxSighashes() {
	require := s.Require()
	tx := tx.Tx{
		Input: &tx_input.TxInput{},
	}
...
func (s *ChainkitTestSuite) TestTxAddSignature() {
	require := s.Require()
	asset := &xc.ChainConfig{Chain: xc.BTC, Network: "testnet"}
	builder, _ := NewTxBuilder(asset)
	from := xc.Address("mpjwFvP88ZwAt3wEHY6irKkGhxcsv22BP6")
